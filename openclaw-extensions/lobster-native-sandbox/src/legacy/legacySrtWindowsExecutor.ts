...
export interface LegacySrtWindowsExecutorOptions {
  helperPath: string;
...
export class LegacySrtWindowsExecutor implements NativeSandboxExecutor {
  ...
  constructor(private readonly options: LegacySrtWindowsExecutorOptions) {
  ...
  createFsIo(params: {
    ...
    return new LegacySrtSandboxFsIo({
      executor: this,
  ...
  async prepareWorkspace(rawWorkspaceDir: string): Promise<void> {
    ...
    const flight = (async () => {
      ...
      try {
        ...
        this.lastErrorCode = error instanceof LobsterNativeSandboxBackendError
          ? error.code
        ...
        if (error instanceof LobsterNativeSandboxBackendError) {
          throw error;
        }
        throw new LobsterNativeSandboxBackendError(
          LobsterNativeSandboxBackendErrorCode.RuntimeInitializationFailed,
  ...
  async wrapCommand(params: {
    ...
    try {
      ...
      const token: NativeSandboxCommandToken = {
        id: createCommandId(),
        command: params.command,
        sessionKey: params.sessionKey,
        startedAt,
  ...
  async finalizeCommand(params: {
    ...
    const token = params.token as NativeSandboxCommandToken | undefined;
    try {
      this.manager.cleanupAfterCommand();
    } finally {
      if (token) {
  ...
  async reset(): Promise<void> {
    ...
    const flight = this.manager.reset()
      ...
      .catch((error) => {
        this.state = LobsterNativeSandboxRuntimeState.Error;
        this.lastErrorCode = LobsterNativeSandboxBackendErrorCode.RuntimeInitializationFailed;
        this.options.audit.record({
          type: SandboxAuditEventType.BackendFailedClosed,
          result: SandboxAuditResult.Failed,
          workspaceDir,
  ...
  private assertRuntimeAvailable(): void {
    ...
    if (this.platform !== 'win32') {
      throw new LobsterNativeSandboxBackendError(
        ...
        'The legacy native-sandbox adapter only supports Windows.',
      );
    }
    if (
  ...
  private assertSameWorkspace(workspaceDir: string): void {
    ...
    if (activeWorkspace.toLowerCase() !== workspaceDir.toLowerCase()) {
      this.options.audit.record({
        ...
        workspaceDir,
        errorCode: LobsterNativeSandboxBackendErrorCode.WorkspaceConflict,
      });
      throw new LobsterNativeSandboxBackendError(
  ...
  private spawnCaptured(params: {
    ...
    if (params.signal?.aborted) {
      return Promise.reject(new LobsterNativeSandboxBackendError(
        LobsterNativeSandboxBackendErrorCode.CommandExecutionFailed,
    ...
    return new Promise<NativeSandboxCommandResult>((resolve, reject) => {
      ...
      const onAbort = (): void => {
        child.kill();
        fail(new LobsterNativeSandboxBackendError(
          LobsterNativeSandboxBackendErrorCode.CommandExecutionFailed,
      ...
      const append = (target: Buffer[], chunk: Buffer, isStdout: boolean): void => {
        ...
        if (nextBytes > MAX_CAPTURE_BYTES) {
          child.kill();
          fail(new LobsterNativeSandboxBackendError(
            LobsterNativeSandboxBackendErrorCode.CommandOutputLimitExceeded,
      ...
      child.once('error', error => fail(new LobsterNativeSandboxBackendError(
        LobsterNativeSandboxBackendErrorCode.CommandExecutionFailed,
...
export const LEGACY_SRT_RUNTIME_METADATA = {
  policyVersion: LOBSTER_NATIVE_POLICY_VERSION,
  runtimeVersion: LEGACY_SRT_RUNTIME_VERSION,
} as const;
