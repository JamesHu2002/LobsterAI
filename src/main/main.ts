...
if (!gotTheLock) {
  ...
  ipcMain.handle('auth:getModels', async () => {
    try {
      ...
      const data = await resp.json() as { code: number; data: Array<{ modelId: string; modelName: string; provider: string; apiFormat: string; supportsImage?: boolean; explicitContextCache?: boolean }> };
