...
import { resolveNativeSandboxHelperPath } from './nativeSandboxEnvironment';
...
const createService = (
  ...
  overrides: Partial<ConstructorParameters<typeof NativeSandboxService>[0]> = {},
) => new NativeSandboxService({
  appPath: 'C:\\LobsterAI',
...
describe('resolveNativeSandboxHelperPath', () => {
  test('uses a direct resources path in packaged builds', () => {
    expect(resolveNativeSandboxHelperPath({
      appPath: 'C:\\LobsterAI',
      resourcesPath: 'C:\\LobsterAI\\resources',
      isPackaged: true,
      architecture: 'x64',
...
describe('NativeSandboxService', () => {
  ...
  test('reports a missing helper without loading the runtime', async () => {
    ...
    expect(result.status.helperAvailable).toBe(false);
    expect(result.status.lastError?.code).toBe(NativeSandboxErrorCode.HelperUnavailable);
    expect(loadRuntime).not.toHaveBeenCalled();
  });
  ...
  test('verifies a complete Windows installation as ready', async () => {
    ...
    expect(result.status.network?.state).toBe(NativeSandboxNetworkState.Verified);
