...
export interface LegacySrtWindowsRuntime {
...
const isLegacySrtWindowsRuntime = (value: unknown): value is LegacySrtWindowsRuntime => {
  if (!value || typeof value !== 'object') return false;
  const module = value as Partial<Record<keyof LegacySrtWindowsRuntime, unknown>>;
...
export const loadLegacySrtWindowsRuntime = async (): Promise<LegacySrtWindowsRuntime> => {
  const loaded = await indirectImport('@anthropic-ai/sandbox-runtime');
  if (!isLegacySrtWindowsRuntime(loaded)) {
