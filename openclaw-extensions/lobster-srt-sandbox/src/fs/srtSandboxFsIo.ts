...
import type { SrtWindowsSession } from '../backend/srtWindowsSession.js';
import {
  SandboxFsError,
  SandboxFsErrorCode,
  throwIfAborted,
...
const POWERSHELL_FILE_HELPER_SHELL: WindowsBinShell = {
  exe: path.win32.join(
...
/**
 * Executes the final file-system mutation/read as the SRT sandbox account.
 ...
 */
export class SrtSandboxFsIo implements SandboxFsIo {
  constructor(
  ...
  private async run(params: {
    ...
    let stagedInput: Awaited<ReturnType<SrtWindowsSession['stageInput']>> | undefined;
    let stagedRequest: Awaited<ReturnType<SrtWindowsSession['stageInput']>> | undefined;
    try {
      stagedInput = params.stdin === undefined
        ? undefined
        : await this.options.session.stageInput({
            data: params.stdin,
            workspaceDir: this.options.workspaceDir,
          });
      stagedRequest = await this.options.session.stageInput({
      ...
      const result = await this.options.session.runIsolatedCommand({
