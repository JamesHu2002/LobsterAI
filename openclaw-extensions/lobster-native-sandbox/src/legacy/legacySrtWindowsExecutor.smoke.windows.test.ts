...
import { LegacySrtWindowsExecutor } from './legacySrtWindowsExecutor.js';
...
test.runIf(runSmoke)(
  ...
  async () => {
    ...
    const runCommand = async (
      stage: string,
      params: Parameters<LegacySrtWindowsExecutor['runIsolatedCommand']>[0],
    ) => {
    ...
    try {
      ...
      const bridge = createWindowsSandboxFsBridge({
        ...
        io: session.createFsIo({
          workspaceDir,
          sessionKey: 'legacy-smoke',
        }),
      });
